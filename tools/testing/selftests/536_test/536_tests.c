...
static uint64_t test_iterate_uds(unsigned int iterations, size_t n_bytes, int sock_type)
{
	...
	/* create socket pair */
	r = socketpair(AF_UNIX, sock_type, 0, uds);
	assert(r >= 0);
		     ...
		     size_t n_bytes)
{
	...
	for (i = 0; i < n_destinations; ++i) {
		...
		r = bus1_ioctl_slice_release(fds[i], (uint64_t *)&cmd_recv.msg.offset);
		assert(r >= 0);
	}
}
			     ...
			     size_t n_bytes)
{
	...
	/* caches */
	test_one(fds[0], fds + 1, handles + 1, n_destinations, payload, n_bytes);
...
static void test_io2(unsigned int number_of_iteration, unsigned long file_size)
{
	unsigned long base;
	int i;
...
static void test_io(void)
{
	...
	fprintf(stderr, "UDS took %lu ns without payload\n",
		test_iterate_uds(100000, 0, SOCK_SEQPACKET));
	fprintf(stderr, "UDS took %lu ns\n",
		test_iterate_uds(100000, 1024, SOCK_SEQPACKET));
